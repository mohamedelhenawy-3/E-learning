...
const updateCourseData=async(req,res,next)=>{
  try{
      ...
      if( req.user._id == course.doctorData.doctorId){
        const updateCourseData=await Course.findOneAndUpdate({"id":course._id},{
          $set:{
               courseName:req.body.courseName,
               description:req.body.description      
          }
...
const updateCourse = async (req, res, next) => {
  ...
  const deleteCourse=async(req,res)=>{
    try{
      ...
      Cloudinary.api.delete_resources_by_prefix(`E-learning/courses/${course.courseName}`, function (err) {
        if (err && err.http_code !== 404) {
            return callback(err);
        }
         Cloudinary.api.delete_folder(`E-learning/courses/${course.courseName}`,function(err,result){
                console.log(err)
            });
          })
      }
...
const getquizResponses = async (req, res) => {
  ...
  const course = await Course.findById(courseId).populate({
    ...
    populate: [
      ...
      { path: "quizId", model: "Quiz" ,populate:{path:'questions' ,model:"Question" ,select:"mark"}},
    ],
  });
