...
router.post("/courses/:courseId/posts/:postId/comments", [auth], async (req, res, next) => {
    try {
      ...
      if (!course) {
        return res.status(404).json({ error: "User is not enrolled in the course." });
      }
...
//dlete commetn by user who create it 
  router.delete("/courses/:courseId/posts/:postId/comments/:commentId", [auth], async (req, res, next) => {
    try {
      const courseId = req.params.courseId;
      const postId = req.params.postId;
      ...
      const comment = await Comment.findOne({ _id: commentId, post: postId, user: userId });
  ...
  router.get("/courses/:courseId/posts/:postId/comments", [auth], async (req, res, next) => {
    try {
      const courseId = req.params.courseId;
      const postId = req.params.postId;
      const userId = req.user.id; // Assuming you have implemented user authentication
